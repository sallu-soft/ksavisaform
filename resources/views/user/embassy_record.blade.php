    ...
    <div class="flex justify-between items-center xl:w-[65%] w-[90%] mx-auto mt-3">
        ...
        <div class="  py-2">
            <form action="{{ route('user/embassy_report') }}" method="GET" class="flex gap-4">
                <input type="date" name="search_date" class="border border-gray-400 p-2 w-[250px] rounded-lg"
                    value="{{ request()->get('search_date') }}" required>
                <button type="submit" class="bg-green-700 px-3 py-2 text-white rounded-lg">Search</button>
            </form>
        </div>
    </div>
    <div class="datelist">
        <table class="table stripe xl:w-[65%] w-[90%] shadow-lg mx-auto mt-10  no-footer dataTable passenger-table"
            id="simpleTable">
                    ...
                    <tr class="bg-gray-700 td-bg my-auto">
                        ...
                        <td class="p-2 flex gap-2 justify-center">
                            <div class="md:text-lg text-md">
                                <a class="bg-blue-700 px-5 py-2 rounded-lg text-white"
                                    href="{{ route('embassy_report_datewise/print', ['date' => $record->record_date]) }}"
                                    class="fw-semibold text-warning">
                                    <i class="bi bi-eye-fill mr-1"></i>View
                                </a>
                            </div>
                            <div class="md:text-lg text-md">
                                <a class="bg-rose-700 px-5 py-2 rounded-lg text-white"
                                    href="{{ route('embassy_report_datewise/agent', ['date' => $record->record_date]) }}"
                                    class="fw-semibold text-warning">
                                    <i class="bi bi-eye-fill mr-1"></i>View with Agent
                                </a>
                            </div>
                            <div class="md:text-lg text-md">
                                <a class="bg-red-700 px-5 py-2 rounded-lg text-white"
                                    href="{{ route('embassy_report_datewise.delete', ['date' => $record->record_date]) }}"
                                    class="fw-semibold text-warning">

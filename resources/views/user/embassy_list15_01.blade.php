...
<body>
    ...
    <div class="container  ">
        ...
        <div class="d-flex max-w-[1050px] mx-auto mt-2 bg-gray-200 rounded-lg px-2 py-1 items-center"
            ...
            {{-- <datalist id="candidates">
                @foreach ($candidates as $index => $candidate)
                    <option data-id="{{ $candidate->candidate_id }}" class="!text-xl ">
                        <b class="font-bold text-3xl">Serial no: {{$candidates->total() - ($loop->index + (($candidates->currentPage() - 1) * $candidates->perPage()))}}</b>,
                        Passport no: {{ $candidate->passport_number }},
                        Candidate Name: {{ $candidate->name }}
                    </option>
                @endforeach
